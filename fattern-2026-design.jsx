...
.f-bg {
  ...
  background:
    ...
    linear-gradient(160deg, #0d1a14 0%, #111820 50%, #0e1118 100%);
  z-index: 0;
}
.f-orb1 {
  position: absolute; width: 320px; height: 320px; border-radius: 50%;
  background: radial-gradient(circle, rgba(45,180,130,0.18) 0%, transparent 70%);
  top: -80px; left: -60px; z-index: 0; filter: blur(40px);
}
.f-orb2 {
  position: absolute; width: 280px; height: 280px; border-radius: 50%;
  background: radial-gradient(circle, rgba(80,140,220,0.15) 0%, transparent 70%);
  bottom: -60px; right: 40px; z-index: 0; filter: blur(50px);
}
...
.f-topbar {
  ...
  background: rgba(10,18,14,0.6); backdrop-filter: blur(30px);
...
.f-brand-mark {
  ...
  box-shadow: 0 0 12px rgba(45,180,130,0.3);
}
.f-brand-mark span { font-size: 11px; font-weight: 700; color: #fff; }
.f-brand-name { font-size: 13px; font-weight: 600; color: rgba(255,255,255,0.9); letter-spacing: -0.01em; }
...
.f-rail {
  ...
  background: rgba(8,15,11,0.5); backdrop-filter: blur(20px);
  border-right: 1px solid rgba(255,255,255,0.06);
}
.f-rail-item {
  width: 36px; height: 36px; border-radius: 10px;
  display: flex; align-items: center; justify-content: center;
  cursor: pointer; color: rgba(255,255,255,0.28); font-size: 17px;
  position: relative; transition: all 0.15s; border: 1px solid transparent;
}
.f-rail-item:hover { background: rgba(255,255,255,0.07); color: rgba(255,255,255,0.65); }
.f-rail-item.active {
  background: rgba(45,180,130,0.18);
  border-color: rgba(45,180,130,0.35);
  color: rgba(80,220,160,0.95);
  box-shadow: 0 0 14px rgba(45,180,130,0.2);
}
.f-rail-badge {
  position: absolute; top: 5px; right: 5px; width: 6px; height: 6px;
  border-radius: 50%; background: #3fd9a0;
  border: 1.5px solid rgba(8,15,11,0.8);
  box-shadow: 0 0 6px rgba(63,217,160,0.6);
}
.f-rail-div { width: 20px; height: 1px; background: rgba(255,255,255,0.07); margin: 4px 0; }
.f-rail-spacer { flex: 1; }

.f-tt-wrap { position: relative; }
.f-tt {
  position: absolute; left: calc(100% + 10px); top: 50%; transform: translateY(-50%);
  background: rgba(20,30,24,0.95); backdrop-filter: blur(16px);
...
.f-tt::before {
  ...
  border: 4px solid transparent; border-right-color: rgba(255,255,255,0.1);
}
...
.f-period { font-size: 10px; font-weight: 500; color: rgba(255,255,255,0.35); letter-spacing: 0.13em; text-transform: uppercase; font-family: 'DM Mono', monospace; margin-bottom: 6px; }
.f-figure { font-size: 44px; font-weight: 700; color: #fff; letter-spacing: -0.04em; line-height: 1; }
.f-figure em { color: #3fd9a0; font-style: normal; }
.f-figure-sub { font-size: 11px; color: rgba(63,217,160,0.7); margin-top: 5px; font-family: 'DM Mono', monospace; }
.f-hero-stats { display: flex; flex-direction: column; align-items: flex-end; gap: 10px; }
.f-hstat { text-align: right; }
.f-hstat-val { font-size: 16px; font-weight: 600; color: #fff; letter-spacing: -0.02em; }
.f-hstat-val.warn { color: #f0b840; }
.f-hstat-val.danger { color: #f07860; }
.f-hstat-label { font-size: 10px; color: rgba(255,255,255,0.3); font-family: 'DM Mono', monospace; margin-top: 1px; }
.f-sparkline { display: flex; align-items: flex-end; gap: 3px; height: 42px; }
.f-sbar { flex: 1; border-radius: 2px 2px 0 0; background: rgba(63,217,160,0.35); }
.f-sbar.now { background: rgba(63,217,160,0.9); box-shadow: 0 0 8px rgba(63,217,160,0.5); }
.f-spark-x { display: flex; gap: 3px; border-top: 1px solid rgba(255,255,255,0.06); padding: 6px 0 14px; }
.f-spark-xl { flex: 1; font-size: 9px; color: rgba(255,255,255,0.2); text-align: center; font-family: 'DM Mono', monospace; letter-spacing: 0.05em; }
...
.f-card {
  ...
  box-shadow: 0 4px 16px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.07);
  transition: border-color 0.15s;
}
.f-card:hover { border-color: rgba(255,255,255,0.15); }
.f-shimmer { position: absolute; top: 0; left: 0; right: 0; height: 1px; }
.f-card.cg .f-shimmer { background: linear-gradient(90deg, transparent, rgba(63,217,160,0.5), transparent); }
.f-card.ca .f-shimmer { background: linear-gradient(90deg, transparent, rgba(240,184,64,0.5), transparent); }
.f-card.cn .f-shimmer { background: linear-gradient(90deg, transparent, rgba(255,255,255,0.2), transparent); }
.f-card-label { font-size: 10px; font-weight: 500; color: rgba(255,255,255,0.3); letter-spacing: 0.1em; text-transform: uppercase; font-family: 'DM Mono', monospace; margin-bottom: 8px; }
.f-card-val { font-size: 20px; font-weight: 700; letter-spacing: -0.02em; margin-bottom: 3px; }
.f-card.cg .f-card-val { color: #3fd9a0; }
.f-card.ca .f-card-val { color: #f0b840; }
.f-card.cn .f-card-val { color: #fff; }
.f-card-meta { font-size: 11px; color: rgba(255,255,255,0.3); font-family: 'DM Mono', monospace; }
.f-card.cg .f-card-meta { color: rgba(63,217,160,0.6); }
...
.f-section-title { font-size: 10.5px; font-weight: 600; color: rgba(255,255,255,0.28); letter-spacing: 0.1em; text-transform: uppercase; }
.f-section-link { font-size: 11.5px; color: rgba(63,217,160,0.8); font-weight: 500; cursor: pointer; }

.f-table-wrap {
  border-radius: 12px; overflow: hidden;
  background: rgba(10,18,14,0.65); backdrop-filter: blur(20px);
  border: 1px solid rgba(255,255,255,0.08);
...
.f-th { font-size: 9.5px; font-weight: 600; color: rgba(255,255,255,0.2); letter-spacing: 0.1em; text-transform: uppercase; font-family: 'DM Mono', monospace; }
...
.f-trow:hover { background: rgba(255,255,255,0.04); }
.f-td-num { font-size: 11px; color: rgba(255,255,255,0.28); font-family: 'DM Mono', monospace; }
.f-td-name { font-size: 12.5px; font-weight: 500; color: rgba(255,255,255,0.82); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.f-td-date { font-size: 11px; color: rgba(255,255,255,0.28); font-family: 'DM Mono', monospace; }
.f-td-amount { font-size: 12.5px; font-weight: 600; color: rgba(255,255,255,0.82); text-align: right; font-family: 'DM Mono', monospace; }
...
.f-pill.paid { background: rgba(63,217,160,0.14); color: rgba(80,220,160,0.9); border: 1px solid rgba(63,217,160,0.2); }
.f-pill.sent { background: rgba(80,140,220,0.14); color: rgba(100,160,240,0.9); border: 1px solid rgba(80,140,220,0.2); }
.f-pill.draft { background: rgba(255,255,255,0.05); color: rgba(255,255,255,0.35); border: 1px solid rgba(255,255,255,0.09); }
.f-pill.overdue { background: rgba(240,120,96,0.14); color: rgba(250,140,110,0.9); border: 1px solid rgba(240,120,96,0.2); }

.f-bottom {
  margin-top: auto; padding: 9px 24px;
  border-top: 1px solid rgba(255,255,255,0.06);
  display: flex; align-items: center; gap: 4px; flex-shrink: 0;
  background: rgba(8,14,10,0.55); backdrop-filter: blur(20px);
}
.f-co-pill {
...
.f-co-av {
  ...
  background: rgba(45,180,130,0.25); border: 1px solid rgba(63,217,160,0.3);
  display: flex; align-items: center; justify-content: center;
  font-size: 8px; font-weight: 700; color: #3fd9a0;
}
.f-co-name { font-size: 11px; color: rgba(255,255,255,0.45); font-weight: 500; }
.f-sc { display: flex; align-items: center; gap: 6px; padding: 4px 8px; border-radius: 6px; cursor: pointer; }
.f-sc:hover { background: rgba(255,255,255,0.05); }
.f-sc-label { font-size: 11.5px; color: rgba(255,255,255,0.32); }
.f-sc-key { font-size: 10px; color: rgba(255,255,255,0.2); background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.09); border-radius: 4px; padding: 1px 5px; font-family: 'DM Mono', monospace; }
.f-bar-right { margin-left: auto; display: flex; align-items: center; gap: 6px; font-size: 10.5px; color: rgba(255,255,255,0.18); font-family: 'DM Mono', monospace; }
.f-status-dot { width: 6px; height: 6px; border-radius: 50%; background: #3fd9a0; box-shadow: 0 0 6px rgba(63,217,160,0.7); flex-shrink: 0; }
          ...
          <div className="f-layout">
            ...
            <div className="f-main">
              ...
              <div className="f-cards">
                ...
                <div className="f-card cn"><div className="f-shimmer"/><div className="f-card-label">Innkrevingstakt</div><div className="f-card-val" style={{color:"#f0b840"}}>73%</div><div className="f-card-meta">av fakturert beløp</div></div>
              </div>
